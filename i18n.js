...
(function () {
  ...
  const STATIC = {
    ...
    "ph.s2.p2": "Mostem między nimi jest jeden mały wzór. F = ma widziałeś na górze strony, ale to była tylko zapowiedź do najechania kursorem, a nie coś, czego użyliśmy. Ten jest pierwszym, w który naprawdę wstawimy liczby. Najedź na każdą literę.",
    ...
    "ph.s7.p4": "Zamiast to twierdzić, oto rzecz do pobawienia się, na kluczu i śrubie, zanim wejdzie jakakolwiek anatomia. Ustaw kąt na 90 stopni, a obie odległości będą sobie równe, i to jest obrazek, który rysuje każdy podręcznik. Potem odsuń kąt od 90, nie ruszając dłoni i nie zmieniając siły, i patrz, jak moment spada, podczas gdy <em>r</em> zostaje dokładnie tam, gdzie było. Przy 0 stopniach ciągniesz wzdłuż dźwigni, linia działania siły przechodzi prosto przez oś obrotu, a pociągnięcie siłą 300 N nie daje zupełnie nic.",
    "ph.s7.lesson": "<b>Zdanie warte zabrania ze sobą.</b> Moment nie zależy od tego, jak daleko od osi obrotu chwyciłeś. Zależy od <b>prostopadłej odległości od osi obrotu do prostej, wzdłuż której działa siła</b>. Te dwie wielkości są tą samą liczbą tylko wtedy, gdy ciągniesz pod kątem prostym, a to jest właśnie przypadek rysowany w podręcznikach, i dlatego tak wiele osób kończy kurs fizyki, nigdy nie zauważywszy różnicy.",
    ...
    "fp.label": "Sześć kanałów, osiem decyzji",
    "fp.title": "Laboratorium platformy",
    "fp.sub": "Platforma dynamometryczna nie mierzy skoku. Mierzy trzy siły i trzy momenty, tysiąc razy na sekundę, a każda liczba, którą trener albo klinicysta z niej cytuje, jest czymś, co <b>analityk postanowił policzyć</b>. Tutaj te decyzje podejmujesz sam, po kolei, i patrzysz, jak wynik się przesuwa, gdy zmienisz zdanie.",
    "fp.prov": "<b>Ten zapis jest syntetyczny.</b> To model dydaktyczny, a nie pomiar człowieka, i mówi o tym na każdym ekranie.",
    "fp.honest": "<b>Dlaczego dane są zmyślone i dlaczego akurat tutaj jest to uczciwy wybór.</b> Środka nacisku nie da się policzyć z samych trzech sił. Potrzebne są też momenty płyty i podany układ współrzędnych, a nie znaleźliśmy otwartego zbioru publikującego wszystkie sześć kanałów wraz z jednoznaczną konwencją. Zapis udający pomiar byłby gorszy niż przyznany model na stronie, której całym tematem jest to, skąd biorą się liczby. Więc sygnał jest generowany, i to trudniejszą drogą: przyspieszenie środka masy jest <b>całkowane</b> do prędkości i wysokości, a siła pionowa wynika z drugiej zasady dynamiki. Nic nie jest rysowane tak, by wyglądało przekonująco. Dlatego impuls, który poniżej policzysz, i czas lotu, który zmierzysz, zgadzają się ze sobą, i tylko dlatego mogą.",
    "fp.s1.h": "Zerowanie jest przed wszystkim",
    "fp.s1.p": "Wzmacniacz w tym zapisie ma przesunięcie kilku niutonów, co jest zwyczajne i co ma każda płyta. Jeśli go nie usuniesz, każda podana dalej siła będzie błędna o tę stałą, a błąd schowa się w wynikach, które poza tym wyglądają całkowicie rozsądnie. Oknem, które trzeba uśrednić, jest to, w którym <b>na płycie nikogo nie ma</b>, a nie dowolny spokojnie wyglądający fragment. Pierwsza wersja tej stacji w ogóle nie miała okresu bez obciążenia i sprawdzenie wykryło to natychmiast: student odjąłby 693,70 N zamiast 7 N i wyzerowałby wraz z przesunięciem własny ciężar ciała.",
    "fp.s2.h": "To, gdzie stopa opuszcza płytę, jest Twoją decyzją",
    "fp.s2.p": "W pliku nie ma zapisu „tu nastąpiło odbicie”. Wybierasz siłę, poniżej której uznajesz stopę za oderwaną, a ten wybór jest założeniem, które przyjmujesz o pomiarze, a nie faktem, który z niego odczytujesz. Palce nie odrywają się w jednej próbce: siła najpierw szybko spada, a potem dogasa, więc niski próg ogłasza odbicie później niż wysoki. W tym zapisie przejście z 5 N na 50 N zmienia policzoną wysokość skoku z <b>26,6 cm na 27,4 cm</b>. Osiem milimetrów, wyłącznie z wpisanej liczby.",
    "fp.s3.h": "Dwie drogi do tej samej wysokości",
    "fp.s3.p": "Droga przez impuls sumuje siłę ponad ciężar ciała w czasie i zamienia ją na prędkość odbicia, bo impuls równa się zmianie pędu. Droga przez czas lotu w ogóle ignoruje siłę i korzysta tylko z tego, jak długo człowiek był w powietrzu. Pochodzą z różnych fragmentów sygnału i schodzą się w granicach pół centymetra. Ta zgodność nie jest ozdobą: to sprawdzenie, które mówi, że przetwarzanie wykonano poprawnie, i pierwsza rzecz warta policzenia, gdy platforma poda liczbę, której się nie spodziewałeś.",
    "fp.s4.h": "Kiedy środek nacisku przestaje istnieć",
    "fp.s4.p": "Środek nacisku jest ilorazem, a w jego mianowniku stoi siła pionowa. Gdy człowiek stoi na płycie, punkt siedzi sensownie pod stopami i wędruje centymetr albo dwa wraz z jego balansowaniem. W fazie lotu mianownik dąży do zera, a iloraz ucieka tam, gdzie poniesie go szum, raportując położenia metry poza płytą, która ma pół metra szerokości. <b>Człowiek się nie przesunął.</b> Po prostu nie ma środka nacisku bez sensownego kontaktu, a oprogramowanie, które mimo to rysuje punkt, rysuje artefakt. Przesuń suwak czasu w fazę lotu i zobacz to sam.",
    "fp.s5.h": "Ostatnie pytanie",
    "fp.s5.p": "Załóżmy, że szczytowa siła pionowa przy lądowaniu rośnie między dwiema sesjami z 2,1 do 2,5 ciężaru ciała. Czy obciążenie stawu kolanowego wzrosło w tej samej proporcji? To kuszące, tak właśnie zdaje się mówić ta liczba, i to nie wynika. Platforma mierzy to, co przechodzi między całym ciałem a podłożem. To, co przechodzi przez jeden staw, zależy od ułożenia kończyny i od tego, jak mięśnie podzieliły się pracą, a żadnej z tych rzeczy w tym pliku nie ma. <b>Nie da się rozstrzygnąć</b> jest poprawną odpowiedzią, a umiejętność udzielenia jej z przekonaniem to większość tego, po co jest ta stacja.",
